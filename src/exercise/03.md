...
JSX is basically an abstraction over the raw React API. Allows us to write XML
like code, meaning it allows us to combine our markup and javascript in a very
intuitive, non-complicated way. We can switch between markup and JS inside JSX
by using curly braces. JSX will get compiled by something like babel to form
React.createElement() syntax. React.createElement(elementType, props,
...children) or React.createElement(elementType, props, children) or
React.createElement(elType, props : {...properties, children})
